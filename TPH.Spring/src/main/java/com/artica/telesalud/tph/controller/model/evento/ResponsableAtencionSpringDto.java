...
/**
 * Clase utilizada para mapear responsables de atencion a objetos JSON.
...
public class ResponsableAtencionSpringDto {
	...
	public ResponsableAtencionSpringDto(ResponsableAtencion responsable)
	{
		responsableAtencionId = responsable.getResponsableAtencionId();
		if(responsable.getLesionado() != null)
		{
			lesionado=new LesionadoSpringDto(responsable.getLesionado());
		}
		externo = responsable.getExterno()==0?false:true;
		if(responsable.getPerson() != null)
		{
			person=new PersonSpringDto(responsable.getPerson());
		}

		creator = responsable.getCreator();
		dateCreated = responsable.getDateCreated();
	}
