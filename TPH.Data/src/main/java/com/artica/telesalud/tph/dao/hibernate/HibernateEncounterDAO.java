...
public class HibernateEncounterDAO extends HibernateDAO implements EncounterDAO {
	...
	public Long getTotalEncounters(Patient patient) {
		...
		try {
			session = getNewSession();
			tx = session.beginTransaction();
			Query criteria = session.createQuery("Select count(e) FROM Encounter e WHERE e.patient=:Paciente AND e.voided=0");
	...
	public List<Encounter> getEncounters(Patient patient, Date high, Date low) {
		...
		try {
			...
			session = getNewSession();
				tx = session.beginTransaction();
			Query criteria = session.createQuery("FROM Encounter e WHERE e.patient=:Paciente AND e.voided=0 AND e.encounterDatetime <=:high AND e.encounterDatetime >=:low");
